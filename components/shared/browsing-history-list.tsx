...
function ProductList({ title, type = "history", hideDetails = false, excludeId = "" }: { title: string; type: "history" | "related"; hideDetails?: boolean; excludeId?: string }) {
  const { products } = useBrowsingHistory();
  const [data, setData] = React.useState([]);
  useEffect(() => {
    const fetchProducts = async () => {
      const res = await fetch(
        `/api/products/browsing-history?type=${type}excludeId=${excludeId}&categories=${products.map((product) => product.category).join(",")}&ids=${products.map((product) => product.id).join(",")}`
      );
