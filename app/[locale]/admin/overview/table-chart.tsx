    ...
    <div className="space-y-3">
      {dataWithPercentage.map(
        ({ label, id, value, image, percentage }, index) => (
          console.log({ label, id, value, image, percentage }),
          (
            <div key={id ?? `${label}-${index}`} className="grid grid-cols-[100px_1fr_80px] md:grid-cols-[250px_1fr_80px] gap-2 space-y-4  ">
              {image ? (
                <Link className="flex items-end" href={`/admin/products/${id}`}>
                  <Image className="rounded border  aspect-square object-scale-down max-w-full h-auto mx-auto mr-1" src={image!} alt={label} width={36} height={36} />
