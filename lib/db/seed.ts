...
const main = async () => {
  try {
    ...
    const cleanedData = addressData.map((province) => ({
      ...province,
      Districts: (province.Districts || []).map((district) => ({
        ...district,
        Wards: (district.Wards || []).filter((ward) => ward && typeof ward === "object" && "Id" in ward && "Name" in ward),
      })),
    }));
...
const generateOrder = async (i: number, users: any, products: any): Promise<IOrderInput> => {
  ...
  const product2 = await Product.findById(products[i % products.length >= products.length - 1 ? (i % products.length) - 1 : (i % products.length) + 1]);
  const product3 = await Product.findById(products[i % products.length >= products.length - 2 ? (i % products.length) - 2 : (i % products.length) + 2]);
  ...
  return order; // as unknown as IOrderInput;
};

export const calcDeliveryDateAndPriceForSeed = ({ items, deliveryDateIndex }: { deliveryDateIndex?: number; items: OrderItem[]; shippingAddress?: ShippingAddress }) => {
