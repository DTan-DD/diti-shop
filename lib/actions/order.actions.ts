...
export const createOrderFromCart2 = async (clientSideCart: Cart, userId: string) => {
  const cart = {
    ...clientSideCart,
  ...
  // CRITICAL: Reserve stock ngay sau khi tạo order
  try {
    await reserveStock(createdOrder._id.toString());
  } catch (error) {
    // Nếu reserve fail -> xóa order vừa tạo
...
/**
 * Tạo order mới từ giỏ hàng và reserve stock.
 * - Redis lock theo userId để tránh double order
 * - Rollback nếu reserve thất bại
 * - Thiết lập expiry cho order
 */
export const createOrderFromCart = async (clientSideCart: Cart, userId: string) => {
  if (!(await acquireUserLock(userId))) {
    throw new Error("Another order is being processed for this user.");
  }

  try {
    // 1️⃣ Tính lại giá và ngày giao hàng trên server
    const cart = {
      ...clientSideCart,
      ...calcDeliveryDateAndPrice({
        items: clientSideCart.items,
    ...
    // 2️⃣ Tạo dữ liệu order
    const order = OrderInputSchema.parse({
      user: userId,
      items: cart.items,
      shippingAddress: cart.shippingAddress,
      paymentMethod: cart.paymentMethod,
      itemsPrice: cart.itemsPrice * 25000,
      shippingPrice: cart.shippingPrice ? cart.shippingPrice * 25000 : 0,
      taxPrice: cart.taxPrice ? cart.taxPrice * 25000 : 0,
    ...
    // 5️⃣ Đặt expiry 30 phút
    // const expiry = new Date();
    // expiry.setMinutes(expiry.getMinutes() + 30);
    // await Order.updateOne({ _id: createdOrder._id }, { stockReservationExpiry: expiry });
...
export async function cancelOrder(orderId: string) {
  try {
    ...
    // Cập nhật order
    order.isCancelled = true; // Cần thêm field này vào schema
...
export const calcDeliveryDateAndPrice = async ({ items, shippingAddress, deliveryDateIndex }: { deliveryDateIndex?: number; items: OrderItem[]; shippingAddress?: ShippingAddress }) => {
...
export const sendMail = async (order: IOrder) => {
  try {
    const data = await Order.findById(order._id).populate("user", "email");
    await sendPurchaseReceipt({ order: data! });
