...
export const OrderItemSchema = z.object({
  ...
  availableStock: z.number().int().nonnegative("Quantity must be a non-negative number"),
  reservedStock: z.number().int().nonnegative("Quantity must be a non-negative number"),
...
export const ShippingAddressSchema = z.object({
  ...
  ward: z.string().min(1, "Vui lòng chọn Phường/Xã nha"),
...
export const OrderInputSchema = z.object({
  ...
  stockStatus: z.enum(["reserved", "confirmed", "released"]).default("reserved"),
  stockReservedAt: z.date().optional(),
  stockReservationExpiry: z.date().optional(),
  isCancelled: z.boolean().default(false),
  cancelledAt: z.date().optional(),
  createdAt: z.date().optional(), // ✅ thêm dòng này
});
...
// USER
const UserName = z.string().min(2, { message: "Username must be at least 2 characters" }).max(50, { message: "Username must be at most 30 characters" });
const Email = z.string().min(1, "Email is required").email("Email is invalid");
const Password = z.string().min(3, "Password must be at least 3 characters");
const UserRole = z.string().min(1, "role is required");
const Phone = z.string().regex(/^0\d{9}$/, { message: "Phone must be 10 digits and start with 0" });
...
export const DeliveryDateSchema = z.object({
  ...
  freeShippingMinPrice: z.coerce.number().min(0, "Free shipping min amount must be at least 0"),
});
...
export const SettingInputSchema = z.object({
  ...
  common: z.object({
    pageSize: z.coerce.number().min(1, "Page size must be at least 1").default(9),
    isMaintenanceMode: z.boolean().default(false),
    freeShippingMinPrice: z.coerce.number().min(0, "Free shipping min price must be at least 0").default(0),
    defaultTheme: z.string().min(1, "Default theme is required").default("light"),
    defaultColor: z.string().min(1, "Default color is required").default("gold"),
    addressVersion: z.string().default("v1.0.0").optional(), // ✅ thêm dòng này
  }),
  site: z.object({
    name: z.string().min(1, "Name is required"),
  ...
  carousels: z.array(CarouselSchema).min(1, "At least one language is required"),
  ...
  availableDeliveryDates: z.array(DeliveryDateSchema).min(1, "At least one delivery date is required"),
  defaultDeliveryDate: z.string().min(1, "Delivery date is required"),
});

// OTP
export const OTPVerificationSchema = z.object({
  otp: z.string().length(6, "Mã OTP phải có 6 số").regex(/^\d+$/, "Mã OTP chỉ chứa số"),
});
